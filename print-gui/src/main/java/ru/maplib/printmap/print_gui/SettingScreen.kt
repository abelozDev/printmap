package ru.maplib.printmap.print_gui
...
import ru.avangard.print_gui.R
import ru.maplib.printmap.print_gui.utils.formatSize
...
fun DownloadMapSettingsScreen(
    ...
    var tilesSize by rememberSaveable { mutableStateOf<Int?>(null) }
    var fileSize by rememberSaveable { mutableStateOf<Int?>(null) }
    var currentZoom by rememberSaveable { mutableFloatStateOf(zoom.toFloat()) }
    ...
    Scaffold(modifier = Modifier.fillMaxSize()) { innerPadding ->
        Column(
            ...
            tilesSize?.let {
                Text(
                    text = stringResource(R.string.tiles_count, it)
                )
                Spacer(Modifier.height(16.dp))
            }
            fileSize?.let {
            ...
            Text(
                ...
                text = currentZoom.roundToInt().toString()
            )
            Slider(
                value = currentZoom,
                onValueChange = {
